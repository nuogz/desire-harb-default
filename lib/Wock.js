...
class WockConnection {
	constructor(wock, wocker, $, maresClose) {
		...
		wock.on('close', (wock, code, reason) => this.handleClose(`[代码]{${code || 0}}, [原因]{${reason || ''}}`));
...
class Wocker {
	constructor($, maresUpgrade = [], maresClose = []) {
		const { C: { wock: { route } }, server } = $;

		const serverWock = this.server = new WebSocket.Server({
			noServer: true,
			perMessageDeflate: {
				zlibDeflateOptions: {
					chunkSize: 1024,
		...
		serverWock.on('connection', wock => new WockConnection(wock, this, $, maresClose));
